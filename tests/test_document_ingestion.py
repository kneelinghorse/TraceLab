...
LONG_TEXT = ("The quick brown fox jumps over the lazy dog near the UX research lab. " * 60).strip()
...
def build_csv(path: Path, text: str) -> Path:
    ...
    df = pd.DataFrame(
        [{"question": f"Q{i+1}", "response": sentence} for i, sentence in enumerate(sentences)]
    )
...
def test_ingestion_pipeline_handles_all_formats(format_name: str, builder, db_session, project, tmp_path):
    ...
    result = ingestion_service.process_document(db=db_session, document_id=document.id, file_path=file_path)
    ...
    statuses = db_session.query(DocumentProcessingStatus).filter(
        DocumentProcessingStatus.document_id == document.id
    ).all()
    succeeded_stages = {
        (status.stage, status.status) for status in statuses
    }
    ...
    format_key = persisted.file_path and Path(persisted.file_path).suffix.lstrip(".").upper()
...
async def test_upload_and_process_endpoints_record_statuses(db_session, project, tmp_path, monkeypatch):
    ...
    try:
        ...
        async with AsyncClient(transport=transport, base_url="http://test") as client:
            ...
            assert any(event["stage"] == "uploaded" for event in uploaded_payload["processing_events"])

            document_id = uploaded_payload["id"]

            process_response = await client.post(f"{documents_api.settings.api_v1_prefix}/documents/{document_id}/process")
            assert process_response.status_code == 200
            process_payload = process_response.json()
            assert process_payload["status"] == "completed"

            detail_response = await client.get(f"{documents_api.settings.api_v1_prefix}/documents/{document_id}")
            assert detail_response.status_code == 200
            detail_payload = detail_response.json()
            succeeded = {event["stage"] for event in detail_payload["processing_events"] if event["status"] == "succeeded"}
