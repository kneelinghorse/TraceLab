...
from app.models.processing_status import DocumentProcessingStatus as ProcessingStatusModel
...
def test_embedding_stage_upserts_vectors_and_sets_metrics(db_session, project, tmp_path):
    ...
    statuses = db_session.query(ProcessingStatusModel).filter(
        ProcessingStatusModel.document_id == document.id, ProcessingStatusModel.stage == "embedded"
    ).all()
    status_values = {entry.status for entry in statuses}
    assert status_values == {"in_progress", "succeeded"}


def test_embedding_stage_records_failure_without_corrupting_document(db_session, project, tmp_path):
    ...
    assert ("in_progress", "Generating embeddings and upserting to Qdrant") in status_pairs
