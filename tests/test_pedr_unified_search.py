...
class _GraphRecorder:
    def __init__(self, layer_result: LayerResult | None = None) -> None:
...
class TestPEDRSearchOrchestrator:
    ...
    def test_search_metadata(self, mock_lexical, mock_semantic):
        ...
        assert response.metadata.intent in (
            "search",
            "create",
            "update",
            "delete",
    ...
    def test_cache_key_includes_layer_enable_flags(self):
        ...
        with patch(
            "app.services.pedr.search_orchestrator.get_pedr_cache", return_value=cache
        ):
            orchestrator.search(
    ...
    def test_search_applies_syntactic_type(self, mock_lexical, mock_semantic):
        ...
        assert (
            response.metadata.detected_type == "mission"
            or response.metadata.type_confidence > 0
        )
    ...
    def test_post_processing_fuses_boosts_from_base_score(self):
        ...
        lexical_search = MagicMock(
            return_value=[{"chunk_id": "stub", "content": "stub", "score": 0.9}]
        )
        semantic_search = MagicMock(return_value=[])
        ...
        def _syntactic_apply(results, **_kwargs):
            ...
            for row in results:
                ...
                if item["chunk_id"] == "c1":
                    item["type_boost"] = 0.5
                    item["combined_score"] = (
                        float(item.get("combined_score") or 0.0) * 1.5
                    )
                else:
                    item["type_boost"] = 0.0
                out.append(item)
        ...
        def _pragmatic_apply(results, **_kwargs):
            ...
            for row in results:
                ...
                if item["chunk_id"] == "c1":
                    ...
                    item["combined_score"] = (
                        float(item.get("combined_score") or 0.0) * 2.2
                    )
                out.append(item)
            return out
    ...
    def test_graph_layer_enabled_includes_results(self, mock_lexical, mock_semantic):
        ...
        response = orchestrator.search(
            query="graph enabled query", enable_graph=True, top_k=10
        )
    ...
    def test_graph_weights_normalized_and_back_compat(
        self, mock_lexical, mock_semantic
    ):
        ...
        response_no_graph = orchestrator.search(
            query="graph weights baseline", enable_graph=False
        )
        weights_no_graph = response_no_graph.metadata.layer_weights
        assert weights_no_graph["lexical"] == pytest.approx(0.25, rel=1e-3)

        response_graph = orchestrator.search(
...
class TestPEDRIntegration:
    ...
    @pytest.mark.skip(
        reason="Requires OpenAI + Qdrant services (orchestrator init fails without them)"
    )
...
class TestRRFAlgorithmCorrectness:
    ...
    def test_overlap_beats_single_layer(self):
        ...
        fusion = RRFFusion()
        layer1 = LayerResult(
            "l1", results=[{"chunk_id": "overlap"}, {"chunk_id": "only1"}]
        )
        layer2 = LayerResult(
            "l2", results=[{"chunk_id": "overlap"}, {"chunk_id": "only2"}]
        )
