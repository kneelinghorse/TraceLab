...
- **Root & www:** `tracelab.aquex.ai` and `www.tracelab.aquex.ai` are proxied (orange-cloud) CNAMEs that terminate at `frontend-production-43c3.up.railway.app`. Keep SSL/TLS mode on **Full (Strict)** so Cloudflare validates Railway's managed certificate. Run `dig tracelab.aquex.ai CNAME +short` (should surface the Railway target behind Cloudflare flattening) and `curl -I https://tracelab.aquex.ai/missions` (expect `HTTP/2 200`).
- **API subdomain:** `api.tracelab.aquex.ai` proxies to `tracelab-production.up.railway.app` without rewriting the path prefix. Validate health with `curl https://api.tracelab.aquex.ai/api/v1/health` (adjust the `/api/v1` segment to match `NEXT_PUBLIC_API_PATH_PREFIX`) and confirm `access-control-allow-origin: https://tracelab.aquex.ai` is present on the CORS pre-flight response.
- **Allowed origins:** When promoting a new vanity domain, append both `https://<domain>` and `https://www.<domain>` to `CORS_ALLOWED_ORIGINS_PROD` so the FastAPI middleware mirrors Cloudflare's hostnames. Production smoke tests (`PLAYWRIGHT_BASE_URL=https://tracelab.aquex.ai PLAYWRIGHT_API_BASE_URL=https://api.tracelab.aquex.ai PLAYWRIGHT_SKIP_SERVER=1 npx playwright test tests/e2e/production-smoke.spec.ts`) provide an automated proof that DNS, TLS, and the API are in sync.
