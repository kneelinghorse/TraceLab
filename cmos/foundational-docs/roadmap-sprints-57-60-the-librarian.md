...
### Sprint 59 — Q&A, and the search question answered (opened 2026-09-23 by MCP-5; the Q&A missions are scoped with Derek)
...
**The truncation is diagnosed, and the default is raised.** `app/core/config.py:50` set `rag_default_max_tokens = 350` — roughly 260 words — and the frontend never sends `max_tokens`, so every synthesis used that hard default. Not a rendering or streaming bug; a config default nobody revisited. PR #348 (`4feb3e3`, 2026-09-18) raised it to 1500; this section and next-step #401 kept saying 350 until the 2026-09-23 review checked the claim against the tree. Open design point, Derek's: whether it stays a single default or becomes per-request, since a one-line answer and a full synthesis want different budgets.

**Already in the sprint (2026-09-23).** MCP-5 released @aquex/tracelab-mcp 2.0.0 on Derek's "i do want the mcp updated" (decision #535). He published it; the published package passed its end-to-end smoke (receipt: `cmos/reports/sprint-59/MCP-5/`). MCP-6 brings corpus Q&A to the MCP (decision #536, Derek: "yes, lets add it to s59"). It is one action over the same Q&A service as the Librarian, held to the same citation rule and RBAC scope. The Librarian's conversation stays REST-only (LIB-1), and MCP-6's criteria are finalized with the Q&A design.
