...
**What shipped so far:** HYG-2 (PR #338, merged `709670c`, production verified serving that commit). Its two findings changed what the work was. Next-step #368's stated blocker was **false** — Alembic does not re-run an applied revision, proven against real PostgreSQL 15 — so a sprint of deferral bought nothing; and #369's stated cause was a misattribution that was concealing two tests skipped for a dependency problem that no longer exists, one of which passes today and is now live again. RBAC-5 is in PR #339.

**Still Derek's:** creating the permanent verification principals once, on purpose, and setting the secret — then re-arming the schedule. That this happens once by hand rather than every six hours by cron is the entire point, so automating it would recreate the defect. There is still no recorded green production `rbac_verify` run since Sprint 49.
