...
**What it cost, and what is still open:** the demotion stopped at `admin` rather than `viewer` on purpose — the fabricated account holds a live `mcp-server` API key and zero Space memberships, so dropping below the privileged tier would have 403'd every project read and silently emptied every project-scoped search. There is still **no recorded green production `rbac_verify` run**; the only agent-reachable credential authenticates solely the retired bootstrap row that this sprint's own guard now refuses, so Derek must create a verification identity and set `RBAC_VERIFY_USERNAME` / `RBAC_VERIFY_PASSWORD`. Until then the new schedule fails every six hours, by design. Two records were corrected: decision #493's "owns 0 projects" is true only of non-deleted projects (the fabricated account is owner-of-record for 1562 documents, 446 reports, 405 missions and 52 collections), and the Sprint 54 SEC-3 receipt's "no writes to production" is wrong.
