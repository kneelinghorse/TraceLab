...
def find_sprint_yaml_files(
    missions_dir: Path, sprint_number: int | None = None
    ...
    for sprint_dir in sprint_dirs:
        ...
        try:
            ...
            print(
                f"Warning: Could not parse sprint number from {sprint_dir.name}",
                file=sys.stderr,
            )
            continue
...
def migrate_sprint(
    ...
    if not db_path.exists():
        ...
        print(
            "Run 'python cmos/scripts/seed_sqlite.py' first to create the database.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    try:
        ...
        for sprint_id, yaml_files in sorted(sprint_yaml_files.items()):
            ...
            if not backlog_missions:
                ...
                print(
                    f"Consider adding them to backlog first or running seed_sqlite.py"
                )
                continue
...
Examples:
  ...
  python scripts/migrate_sprint_yaml_to_db.py --all
    ...
    parser.add_argument(
        "--sprint", type=int, help="Sprint number to migrate (e.g., 20 for Sprint 20)"
    )

    parser.add_argument(
        "--all", action="store_true", help="Migrate all sprint directories"
    )
    ...
    parser.add_argument(
        "--db", type=Path, help="Path to database file (default: <root>/db/cmos.sqlite)"
    )
