...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=CMOS_ROOT,
        help="CMOS directory (default: auto-detected)",
    )
    ...
    start_parser.add_argument(
        "--mission", required=True, help="Mission identifier (e.g. S4.1)"
    )
    start_parser.add_argument(
        "--agent", default="codex", help="Agent identifier for session logging"
    )
    start_parser.add_argument(
        "--summary", required=True, help="Session summary message"
    )
    ...
    complete_parser.add_argument(
        "--notes", required=True, help="Notes to persist in backlog and DB"
    )
    complete_parser.add_argument("--ts", help="ISO timestamp (UTC). Defaults to now.")
    complete_parser.add_argument(
        "--next-hint", help="Override next_hint field in session log"
    )
    complete_parser.add_argument(
        "--no-promote",
        action="store_true",
    ...
    complete_parser.add_argument(
        "--immediate",
        action="store_true",
        help="Promote next mission directly to In Progress",
    )
    ...
    block_parser.add_argument(
        "--reason", required=True, help="Reason to persist in notes and metadata"
    )
    block_parser.add_argument(
        "--need",
        action="append",
        default=[],
        help="Add a required follow-up item (can be repeated)",
    )
    block_parser.add_argument("--ts", help="ISO timestamp (UTC). Defaults to now.")
    block_parser.add_argument(
        "--next-hint", help="Follow-up hint to include in the session log"
    )

    status_parser = subparsers.add_parser(
        "status", help="Show the highest priority mission candidates"
    )
    status_parser.add_argument(
        "--limit", type=int, default=5, help="Limit number of missions displayed"
    )
...
def handle_complete(args: argparse.Namespace) -> None:
    ...
    output = {"event": result.event, "next_mission": result.next_mission}
