// MCP-5 fresh-install smoke for 2.0.0 (adapted from Sprint 52's MCP-3 smoke, whose
// 1.2.0 assertions no longer hold): start the PUBLISHED @aquex/tracelab-mcp@<version> with
...
const VERSION = process.env.MCP_VERSION || '2.0.0';
...
const out = process.env.UI_OUT || path.join(os.tmpdir(), 'tracelab-mcp5-smoke');
...
const client = new Client({ name: 'mcp-5-npx-smoke', version: '1.0.0' });
const links = [];
...
try {
  ...
  record('published package starts and lists tools', tools.length === 9 && actions === 49 && serverInfo?.version === VERSION, { tools: tools.length, actions, server_version: serverInfo?.version ?? null, startup_stderr: stderr.replace(creds.key, '[redacted]').trim().split('\n').slice(0, 3) });
...
const report = {
  mission_id: 'MCP-5', sprint_id: 'sprint-59', started_at: started, finished_at: new Date().toISOString(), version: VERSION, api, frontend: web, read_only: true,
...
await fs.writeFile(path.join(out, 'mcp-5-production-smoke.json'), JSON.stringify(report, null, 2));
