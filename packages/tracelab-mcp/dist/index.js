...
const TOOLS = [
    ...
    // ─────────────────────────────────────────────────────────────────────────
    {
        ...
        description: 'Semantic search across the TraceLab knowledge base. Returns ranked chunks with content excerpts and document references. Actions: knowledge (find chunks matching a natural-language query). Required for action="knowledge": query. Related clusters: tracelab_document (read full text), tracelab_collection (organize chunks).',
        inputSchema: {
            type: 'object',
            properties: {
                action: {
    ...
    // ─────────────────────────────────────────────────────────────────────────
    {
        ...
        description: 'Projects organize documents and research. Actions: list (browse), create (new project), update (edit metadata), stats (aggregated counts: documents, chunks, reports, tokens). Required: action="create" needs name; action="update"/"stats" needs project_id.',
        inputSchema: {
            type: 'object',
            properties: {
                action: {
    ...
    // ─────────────────────────────────────────────────────────────────────────
    {
        ...
        description: 'Collections group related chunks for synthesis. Actions: list, get (with chunks), export (as markdown), create, add (a chunk), synthesize (generate summary/report from chunks; optional save as report). Required: action="get"/"export" needs collection_id; action="create" needs name; action="add" needs collection_id+chunk_id; action="synthesize" needs collection_id.',
        inputSchema: {
            type: 'object',
            properties: {
                action: {
    ...
    // ─────────────────────────────────────────────────────────────────────────
    {
        ...
        description: 'Document upload + retrieval. Actions: upload (new doc through ingestion pipeline; supports PDF, DOCX, PPTX, CSV, XLSX, MD, TXT, JSON, XML, YAML), get_content (paginated full text assembled from chunks). Required: action="upload" needs name, content (base64), content_type, project_id; action="get_content" needs document_id.',
        inputSchema: {
            type: 'object',
            properties: {
                action: {
    ...
    // ─────────────────────────────────────────────────────────────────────────
    {
        ...
        description: 'Research mission CRUD. Use tracelab_mission_execution for submit/status/preview (DeepSearch-bound lifecycle). Actions: create, list, get, update. project_id is required at create (T41.6) and editable on update (T41.5). action="get" returns slim payload by default; pass include_execution_metadata=true for full execution_metadata/result_protocol/result_markdown (T41.4). Required: action="create" needs mission_id, title, objective, success_criteria, project_id; action="get"/"update" needs mission_id. Related cluster: tracelab_mission_execution.',
        inputSchema: {
            type: 'object',
            properties: {
                action: {
...
function summarizeBlob(value, fieldName) {
...
function maybeSummarizeBlob(value, fieldName) {
    ...
    if (typeof value === 'object' && value !== null && Object.keys(value).length === 0)
        return value;
    const serialized = JSON.stringify(value) ?? '';
    if (Buffer.byteLength(serialized, 'utf8') <= LARGE_BLOB_THRESHOLD_BYTES)
        return value;
    return summarizeBlob(value, fieldName);
}
function maybeSummarizeMarkdown(value) {
    if (value === null || value === undefined || value === '')
        return value;
    if (Buffer.byteLength(value, 'utf8') <= LARGE_BLOB_THRESHOLD_BYTES)
        return value;
...
export async function handleListMissions(args) {
...
export async function handleGetMission(args) {
    ...
    const resultProtocol = slim
        ? maybeSummarizeBlob(result.result_protocol, 'result_protocol')
...
function unknownAction(tool, action, valid) {
    return {
        content: [
            {
                type: 'text',
...
const SEARCH_ACTIONS = ['knowledge'];
export async function handleTracelabSearch(args) {
...
const PROJECT_ACTIONS = ['list', 'create', 'update', 'stats'];
export async function handleTracelabProject(args) {
    const action = getAction(args);
    switch (action) {
        case 'list':
...
export async function handleTracelabCollection(args) {
    const action = getAction(args);
    switch (action) {
        case 'list':
...
const REPORT_ACTIONS = ['create', 'list', 'get', 'export'];
export async function handleTracelabReport(args) {
    const action = getAction(args);
    switch (action) {
        case 'create':
...
const DOCUMENT_ACTIONS = ['upload', 'get_content'];
export async function handleTracelabDocument(args) {
    const action = getAction(args);
    switch (action) {
        case 'upload':
...
const MISSION_ACTIONS = ['create', 'list', 'get', 'update'];
export async function handleTracelabMission(args) {
    const action = getAction(args);
    switch (action) {
        case 'create':
...
const MISSION_EXECUTION_ACTIONS = ['submit', 'status', 'preview'];
export async function handleTracelabMissionExecution(args) {
    const action = getAction(args);
    switch (action) {
        case 'submit':
        ...
        default:
            return unknownAction('tracelab_mission_execution', action, MISSION_EXECUTION_ACTIONS);
    }
}
                    ...
                    if (migrated) {
                        return {
                            content: [
                                {
                                    type: 'text',
                                    text: `Tool "${name}" was renamed in T41.7 (sprint-41). ` +
