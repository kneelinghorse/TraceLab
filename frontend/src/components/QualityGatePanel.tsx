...
const STATUS_TOKENS: Record<string, { label: string; className: string; icon: string }> = {
  pass: { label: "Pass", className: "border-success-line bg-success-surface text-success", icon: "✓" },
  fail: { label: "fail", className: "border-danger-line bg-danger-surface text-danger", icon: "!" },
  pending: { label: "Pending", className: "border-warning-line bg-warning-surface text-warning", icon: "…" },
};
...
export function QualityGatePanel({ report, mission }: Props) {
  ...
  return (
    <div className="rounded-3xl border border-line bg-surface p-6 text-foreground shadow-sm">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.3em] text-muted">Quality gates</p>
          <h3 className="text-2xl font-semibold text-foreground">Research readiness</h3>
        </div>
        <div className={`text-sm font-medium ${failing === 0 ? "text-success" : "text-danger"}`}>
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="text-sm text-secondary">{GATE_DESCRIPTIONS[gate.name]}</p>
                </div>
              ...
              <div className="mt-3 text-sm text-secondary">
                ...
                <p className="mt-1 text-xs text-muted">
                  ...
                  {gate.details && <span className="ml-1 text-danger">· {gate.details}</span>}
                </p>
              </div>
            </div>
          );
