...
class ExecutionMetadata(BaseModel):
    ...
    loops_executed: int | None = Field(
        default=None, description="Number of research loops executed"
    )
    sources_found: int | None = Field(
        default=None, description="Total sources discovered"
    )
    duration_seconds: float | None = Field(
        default=None, description="Total execution time"
    )
    model_used: str | None = Field(
        default=None, description="AI model used for execution"
    )
    pedr_checked: bool | None = Field(
        default=None, description="Whether PEDR was checked"
    )
    pedr_reused: bool | None = Field(
        default=None, description="Whether PEDR cache was reused"
    )
...
class WebhookResponse(BaseModel):
    ...
    received: bool = Field(
        default=True, description="Acknowledgment of webhook receipt"
    )
    mission_id: str | None = Field(
        default=None, description="Mission ID that was updated"
    )
    status: str | None = Field(default=None, description="New mission status")
...
class WebhookErrorResponse(BaseModel):
    ...
    error_code: str | None = Field(default=None, description="Error classification")
