...
class DeepSearchStatusResponse(BaseModel):
    ...
    error_code: str | None = Field(default=None, description="Error code (when failed)")
...
class DeepSearchIngestRequest(BaseModel):
    ...
    mission: MissionProtocolComplete = Field(
        ..., description="MissionProtocolComplete JSON payload"
    )
    project_id: UUID | None = Field(
        default=None, description="Existing TraceLab project UUID"
    )
...
class AutoLinkingMatch(BaseModel):
    ...
    error_type: str | None = Field(
        default=None, description="Error classification if failed"
    )
...
class AutoLinkingError(BaseModel):
    ...
    error_type: str = Field(
        ..., description="Error classification from AutoLinkErrorType"
    )
    ...
    threshold: float | None = Field(
        default=None, description="Similarity threshold for linking"
    )
...
class AutoLinkingSummary(BaseModel):
    ...
    failed: conint(ge=0) = Field(
        default=0, description="Count of failed auto-link attempts"
    )
    threshold: confloat(ge=0.0, le=1.0) = 0.7
    success_rate: confloat(ge=0.0, le=1.0) = 0.0
    failure_rate: confloat(ge=0.0, le=1.0) = Field(
        default=0.0, description="Ratio of failed items"
    )
    matches: list[AutoLinkingMatch] = Field(default_factory=list)
    errors: list[AutoLinkingError] = Field(
...
class CorrectionQueueInfo(BaseModel):
    ...
    queued_count: conint(ge=0) = Field(
        default=0, description="Number of items queued for retry"
    )
    correction_ids: list[UUID] = Field(
        default_factory=list, description="IDs of queued corrections"
    )
    callback_url: str | None = Field(
...
class DeepSearchIngestResponse(BaseModel):
    ...
    corrections: CorrectionQueueInfo | None = Field(
        default=None,
