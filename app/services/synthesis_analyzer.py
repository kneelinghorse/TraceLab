...
class SynthesisAnalyzer:
    ...
    def __init__(
        self, *, min_insight_chars: int = 80, min_recommendation_ratio: float = 0.6
    ) -> None:
    ...
    def evaluate(self, mission: MissionProtocolDraft) -> QualityAutomationCheckResult:
        ...
        if not synthesis:
            ...
            recommendations.append(
                "Populate synthesis.key_insights, recommendations, and next_steps."
            )
        ...
        shallow = [
            insight for insight in insights if len(insight) < self.min_insight_chars
        ]
        if shallow:
            issues.append(
                QualityIssue(
            ...
            recommendations.append(
                "Expand insights with context, tension, and a 'so what'."
            )
        ...
        if rec_count < required_recs:
            ...
            recommendations.append(
                "Tie each key insight to at least one recommendation."
            )
        ...
        if weak_steps:
            ...
            recommendations.append(
                "Lead next steps with strong verbs and measurable outcomes."
            )
