...
class CoverageReportGenerator:
    ...
    def __init__(self, output_path: Path = Path("cmos/reports/sprint-01/ingestion_format_coverage.json")):
        """
    ...
    def generate_report(self, db: Session) -> Dict[str, Any]:
        ...
        for file_ext, stats in format_stats.items():
            format_name = file_ext.lstrip(".").upper() if file_ext != "unknown" else "UNKNOWN"
            
            success_rate = (stats["success"] / stats["total"] * 100) if stats["total"] > 0 else 0
            processed_rate = (stats["processed"] / stats["total"] * 100) if stats["total"] > 0 else 0
            chunked_rate = (stats["chunked"] / stats["total"] * 100) if stats["total"] > 0 else 0
