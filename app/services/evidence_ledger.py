...
class EvidenceLedgerService:
    ...
    def list_ledger(
        ...
        *,
        ...
        entry_query = _apply_access_filter(db.query(LedgerEntry), entry_access_filter).filter(
            LedgerEntry.project_id == project_id
        )
        note_query = _apply_access_filter(db.query(LedgerNote), note_access_filter).filter(
    ...
    def search(
        ...
        *,
        ...
        """Run project-scoped literal ILIKE search without full-text indexing."""
        escaped = _escape_like(keyword)
        pattern = f"%{escaped}%"
        query = _apply_access_filter(db.query(LedgerEntry), access_filter).filter(
            LedgerEntry.project_id == project_id,
            or_(
