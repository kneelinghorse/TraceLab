...
class GraphRAGHelper:
        ...
        for urn in order:
            ...
            if related:
                related_sorted = sorted(
                    related,
                    key=lambda item: (
                        _format_reference(node_map.get(item[0])),
                        item[1],
                    ),
                )
    ...
    def _hydrate_nodes(
        ...
        *,
        ...
        if mission_ids:
            ...
            ).filter(Mission.mission_id.in_(mission_ids.keys()))
            for row in query.all():
        ...
        if collection_ids:
            ...
            ).filter(Collection.id.in_(collection_ids.keys()))
            for row in query.all():
    ...
    def _score_node(
        ...
        if content_tokens:
            overlap = len(set(query_tokens).intersection(content_tokens)) / len(
                set(query_tokens)
            )
        else:
            overlap = 0.0
    ...
    def _get_encoder(self):
        ...
        if tiktoken is None:
            raise RuntimeError(
                "tiktoken is required for GraphRAGHelper token counting."
            )
        if self._model_name:
            try:
...
def _format_reference(node: GraphNode | None) -> str:
    if node is None:
        return "unknown"
    parsed = URN.parse(node.urn or "")
    entity_id = parsed.entity_id if parsed else node.urn
...
def _format_chunk_evidence(urn: str) -> str | None:
...
def _filter_edges(
    ...
    return [
        edge for edge in edges if edge[0] in allowed_nodes and edge[1] in allowed_nodes
    ]
...
def _topological_order(
    ...
    queue = sorted(
        [urn for urn, degree in in_degree.items() if degree == 0], key=sort_key
    )
    order: list[str] = []
