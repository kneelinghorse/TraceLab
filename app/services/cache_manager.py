...
_BUCKETS: dict[str, _Bucket] = {
    "rag_query_results": _Bucket(
        "rag_query_results",
        CacheConfig("rag_query_results", ttl_seconds=300, maxsize=128),
    ),
    "document_lists": _Bucket(
        "document_lists", CacheConfig("document_lists", ttl_seconds=120, maxsize=512)
    ),
    "project_metadata": _Bucket(
        "project_metadata",
        CacheConfig("project_metadata", ttl_seconds=300, maxsize=256),
    ),
    "quality_gates": _Bucket(
        "quality_gates", CacheConfig("quality_gates", ttl_seconds=60, maxsize=256)
    ),
    "mission_validation": _Bucket(
        "mission_validation",
        CacheConfig("mission_validation", ttl_seconds=30, maxsize=256),
    ),
    "relationship_context": _Bucket(
        "relationship_context",
        CacheConfig("relationship_context", ttl_seconds=300, maxsize=512),
    ),
}
...
class CacheManager:
    ...
    def cached_value(
        self, name: str, key: Hashable, loader: CacheLoader
    ) -> tuple[Any, bool]:
    ...
    def invalidate(
        self, name: str, predicate: Callable[[Hashable], bool] | None = None
    ) -> int:
        namespace = self._namespace(name)
        return namespace.invalidate(predicate)

    def clear(self, targets: Sequence[str] | None = None) -> dict[str, int]:
    ...
    # ------------------------------------------------------------------
    def invalidate_document_lists(self, project_id: str | None = None) -> int:
        if not project_id:
            return self.invalidate("document_lists")
        return self.invalidate(
            "document_lists", predicate=lambda key: key[0] == project_id
        )

    def invalidate_project_metadata(self, project_id: str | None = None) -> int:
        if not project_id:
    ...
    def invalidate_quality_gates(self, mission_id: str | None = None) -> int:
        if not mission_id:
            return self.invalidate("quality_gates")
        return self.invalidate(
            "quality_gates", predicate=lambda key: key[0] == mission_id
        )

    def invalidate_mission_validation(self, mission_id: str | None = None) -> int:
        if not mission_id:
            return self.invalidate("mission_validation")
        return self.invalidate(
            "mission_validation", predicate=lambda key: key[0] == mission_id
        )

    def invalidate_relationship_context(self, mission_id: str | None = None) -> int:
        if not mission_id:
            return self.invalidate("relationship_context")
        return self.invalidate(
            "relationship_context", predicate=lambda key: key[0] == str(mission_id)
        )
