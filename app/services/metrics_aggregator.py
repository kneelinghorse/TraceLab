...
class MetricsAggregator:
    ...
    def __init__(
        ...
        *,
        ...
        self.telemetry_path = Path(telemetry_path) if telemetry_path else repo_root / "telemetry" / "events" / "sprint-04-performance.jsonl"
    ...
    def _cost_overview(self, events: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        embedding_cost = self._sum_cost(events, None, predicate=self._is_embedding_event)
    ...
    def _cache_performance(self) -> Dict[str, Any]:
        ...
        ttl_hit_rates = [cache.get("hit_rate") for cache in ttl_caches if isinstance(cache.get("hit_rate"), (int, float))]
        aggregate = {
            "ttl_cache_count": len(ttl_caches),
            "ttl_average_hit_rate": round(mean(ttl_hit_rates), 3) if ttl_hit_rates else 0.0,
    ...
    def _query_performance(self, events: List[Dict[str, Any]], cache: Dict[str, Any]) -> Dict[str, Any]:
        latencies = [float(event.get("latency_ms")) for event in events if isinstance(event.get("latency_ms"), (int, float))]
        now = datetime.now(timezone.utc)
        last_hour = sum(1 for event in events if event.get("_ts") and event["_ts"] >= now - timedelta(hours=1))
        slow_queries = [self._simplify_event(event) for event in sorted(events, key=lambda e: float(e.get("latency_ms", 0.0)), reverse=True) if event.get("latency_ms")]
    ...
    def _system_health(self, events: List[Dict[str, Any]], cache: Dict[str, Any]) -> Dict[str, Any]:
        telemetry_info = {
        ...
        return {
            ...
            "cache": {
                ...
                "semantic_cache_hit_rate": cache.get("semantic_cache", {}).get("hit_rate", 0.0),
            },
    ...
    def _sum_cost(self, events: List[Dict[str, Any]], threshold: datetime | None, predicate: Callable[[Dict[str, Any]], bool] | None = None) -> float:
        total = 0.0
        for event in events:
    ...
    def _aggregate_costs(self, events: List[Dict[str, Any]], *, key: str) -> List[Dict[str, Any]]:
    ...
    def _daily_trend(self, events: List[Dict[str, Any]], *, days: int) -> List[Dict[str, Any]]:
        if not events:
            return []
        buckets: Dict[str, float] = defaultdict(float)
        ...
        return [{"label": label, "cost_usd": round(total, 6)} for label, total in ordered]

    def _daily_latency_trend(self, events: List[Dict[str, Any]], *, days: int) -> List[Dict[str, Any]]:
        if not events:
            return []
        ...
        try:
            with self.engine.connect() as connection:
                ...
                if backend.startswith("sqlite"):
                    table_count = connection.execute(text("SELECT COUNT(*) FROM sqlite_master WHERE type='table'"))
                    index_count = connection.execute(text("SELECT COUNT(*) FROM sqlite_master WHERE type='index'"))
                else:
    ...
    def _qdrant_health(self) -> Dict[str, Any]:
        ...
        except Exception as exc:  # pragma: no cover - qdrant optional in some environments
            return {"status": "unavailable", "error": str(exc)}
        try:
            collections = service.client.get_collections()
            names = [getattr(collection, "name", None) for collection in getattr(collections, "collections", [])]
            ...
            return {"status": "unavailable", "collection": service.collection_name, "error": str(exc)}
    ...
    def _export_rows(
        ...
        rows.append(ExportRow("costs", "average_cost_per_query", cost.get("average_cost_per_query", 0.0), unit="USD"))
        rows.append(ExportRow("costs", "embedding_cost_usd", cost.get("embedding_cost_usd", 0.0), unit="USD"))
        rows.append(ExportRow("costs", "generation_cost_usd", cost.get("generation_cost_usd", 0.0), unit="USD"))

        rows.append(ExportRow("cache", "semantic_cache_hit_rate", cache.get("semantic_cache", {}).get("hit_rate", 0.0)))
        rows.append(ExportRow("cache", "ttl_average_hit_rate", cache.get("aggregate", {}).get("ttl_average_hit_rate", 0.0)))

        rows.append(ExportRow("performance", "p50_latency_ms", query.get("p50_latency_ms", 0.0), unit="ms"))
        rows.append(ExportRow("performance", "p95_latency_ms", query.get("p95_latency_ms", 0.0), unit="ms"))
        rows.append(ExportRow("performance", "p99_latency_ms", query.get("p99_latency_ms", 0.0), unit="ms"))
        rows.append(ExportRow("performance", "requests_last_hour", query.get("requests_last_hour", 0)))
        ...
        rows.append(ExportRow("system", "telemetry_events", telemetry.get("events_available", 0)))
        return rows
