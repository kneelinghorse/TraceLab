...
class DocumentIngestionService:
    ...
    def __init__(
        ...
        coverage_report_generator: CoverageReportGenerator | None = None,
        redaction_service: Any | None = None,
        ...
        self.coverage_report_generator = (
            coverage_report_generator or CoverageReportGenerator()
        )
    ...
    def process_document(
        ...
        try:
            ...
            if not DocumentParser.is_format_supported(parse_path):
                raise ValueError(
                    f"Unsupported file format: {parse_path.suffix if parse_path else 'unknown'}"
                )
            ...
            self.status_recorder.record(
                ...
                details={
                    "chunk_count": len(chunk_records),
                    "average_tokens": int(
                        sum(c.token_count or 0 for c in chunk_records)
                        / len(chunk_records)
                    )
                    if chunk_records
                    else 0,
            ...
            embedding_conditions_met = (
                ...
                and qdrant_service is not None
            )
            ...
            if embedding_conditions_met:
                ...
                document.embedded = False
                db.commit()
                result["stages"]["embedded"] = {"status": "skipped", "reason": reason}
            else:
                result["stages"]["embedded"] = {
                    "status": "skipped",
            ...
            try:
                ...
                result.setdefault("metrics", {})
                result["metrics"]["edge_materialization_duration_seconds"] = (
                    edge_duration
                )
