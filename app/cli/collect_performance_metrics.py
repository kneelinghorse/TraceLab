    ...
    try:
        ...
        metrics["collection_info"] = {
            ...
            "config": {
                ...
                "on_disk": getattr(collection_info.config.params.vectors, 'on_disk', None),
                "quantization": "scalar_int8" if collection_info.config.quantization_config else None
            }
        }
        ...
        if collection_info.points_count > 0:
            metrics["token_usage"] = {
                ...
                "estimated_total_tokens": collection_info.points_count * avg_tokens_per_chunk,
                "estimated_openai_cost_usd": (collection_info.points_count * avg_tokens_per_chunk * 0.02) / 1_000_000
            }
...
def main():
    ...
    print(f"Collection points: {metrics.get('collection_info', {}).get('points_count', 0)}")
    if metrics.get("query_latency"):
        avg_latency = sum(q["latency_ms"] for q in metrics["query_latency"]) / len(metrics["query_latency"])
